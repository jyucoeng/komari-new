...
# Docker Run
docker run ... ghcr.io/jyucoeng/komari-new:latest
```

> **提示：** 可在 [GitHub Packages](https://github.com/jyucoeng/komari-new/pkgs/container/komari) 查看所有可用版本。
...
environment:
  ...
  # 1.3.x 起管理员账号由首次访问站点的 Web 安装向导创建，无需 ADMIN_USERNAME/ADMIN_PASSWORD
...
docker run -d \
  ...
  ghcr.io/jyucoeng/komari-new:latest
```
...
```bash
git clone https://github.com/jyucoeng/komari-new.git && cd komari-new && sudo bash install.sh
```
...
```bash
# 拉取最新镜像
docker pull ghcr.io/jyucoeng/komari-new:latest
...
# 删除镜像（可选）
docker rmi ghcr.io/jyucoeng/komari-new:latest
...
> 注：1.3.x 起不再使用 `ADMIN_USERNAME` / `ADMIN_PASSWORD` 初始化管理员账号，首次访问站点时通过 Web 安装向导创建。
