...
docker_install() {
    ...
    if [ ! -f "${SCRIPT_SOURCE_DIR}/.env" ]; then
        ...
        hint "Required fields: GH_BACKUP_USER, GH_REPO, GH_PAT,"
        hint "ARGO_DOMAIN, KOMARI_CLOUDFLARED_TOKEN"
        hint "Optional: GH_EMAIL defaults to GH_BACKUP_USER@users.noreply.github.com"
        hint "1.3.x 起管理员账号由首次访问站点的 Web 安装向导创建，无需 ADMIN_USERNAME/ADMIN_PASSWORD"
        echo ""
...
vps_download_komari() {
    ...
    if command -v docker >/dev/null 2>&1; then
        ...
        local ghcr_image="ghcr.io/${KOMARI_SOURCE_REPOSITORY}:${version:-latest}"
...
missing=0
for var in ARGO_DOMAIN KOMARI_CLOUDFLARED_TOKEN; do
    if [ -z "${!var:-}" ]; then
...
# 1.3.x 已移除 terminal_enabled 等全局配置键（改为权限模型），
# 终端/远程功能拦截由 setup-config.sh 生成的 Caddyfile 路径规则负责。
# 1.3.x 显式传入主库路径，避免依赖工作目录。
args=(server -l "$KOMARI_LISTEN_ADDR" -d "${KOMARI_DB_FILE:-${DATA_DIR}/komari.db}")
...
# 1.3.x 起管理员账号由首次访问站点的 Web 安装向导创建，以下变量已废弃：
...
case "${1:-help}" in
    ...
    version)
        ...
        if [ -x "${BIN_DIR}/komari" ]; then
            # 1.3.x 起 komari 已移除 version 子命令，仅显示二进制路径
            echo "Komari binary: ${BIN_DIR}/komari"
        fi
...
reinstall_keep_config() {
    ...
    if command -v docker >/dev/null 2>&1 && docker ps -a --format '{{.Names}}' 2>/dev/null | grep -qx "komari"; then
        ...
        if [ -f "${SCRIPT_SOURCE_DIR}/docker-compose.yml" ]; then
            ...
            local image
            image=$(docker inspect komari --format '{{.Config.Image}}' 2>/dev/null || echo "ghcr.io/jyucoeng/komari-new:latest")
            docker pull "$image"
            docker stop komari 2>/dev/null || true
            docker rm komari 2>/dev/null || true
            if ! docker_compose_available; then
